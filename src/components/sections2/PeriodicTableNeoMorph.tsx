...
const NEO_SHADOW_LIGHT = '#ffffff'
const NEO_SHADOW_DARK = '#0a161a30'
const NEO_SHADOW_SIZE = '2px'
const NEO_SHADOW_BLUR = '2px'
...
function ElementTile({ element }: { element: Element }) {
  ...
  const numberColor = isFluorine ? FLUORINE_TEXT : 
    (isHalogen ? HALOGEN_TEXT : '#666666');

  // Adjust shadows based on element type
  const shadowConfig = isFluorine || isHalogen ? {
    boxShadow: `inset 3px 3px 6px rgba(0,0,0,0.25),
                inset -3px -3px 6px rgba(255,255,255,0.1)`
  } : {
    boxShadow: `inset ${NEO_SHADOW_SIZE} ${NEO_SHADOW_SIZE} ${NEO_SHADOW_BLUR} ${NEO_SHADOW_DARK},
                inset -${NEO_SHADOW_SIZE} -${NEO_SHADOW_SIZE} ${NEO_SHADOW_BLUR} ${NEO_SHADOW_LIGHT}`
  };
  ...
  const tileContent = (
    <motion.div
      whileHover={{ 
        scale: 1.02,
        boxShadow: isFluorine || isHalogen ?
          `inset 4px 4px 8px rgba(0,0,0,0.3),
           inset -4px -4px 8px rgba(255,255,255,0.1)` :
          `inset 6px 6px 14px ${NEO_SHADOW_DARK},
           inset -6px -6px 14px ${NEO_SHADOW_LIGHT}`
      }}
      className="absolute p-[2%] flex flex-col items-center justify-center cursor-pointer transition-all duration-300"
      style={{
        top: `calc(
...
export function PeriodicTableNeoMorph() {
  ...
  return (
    <div className="w-full overflow-x-auto p-[10px]">
      <div 
